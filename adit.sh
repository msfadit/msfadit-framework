...
run() {
        cd ~/msfAdit-framework/.core/ && python3 adit.py --run
}

usage() {
        echo -e $w""
        echo -e $w"{"$B$W" ADIT FRAMEWORK "$w"}"
        echo -e $w""
        echo -e $w" usage with commands: adit <"$g"options"$w">"
        echo -e $w" example: "$g"adit run"$w
        echo -e $w""
        echo -e $g" Options:              Descriptions:"
        echo -e $w"---------------------------------------------------"
        echo -e $w" run                   menjalankan adit framework"
...
install() {
        echo
        echo -e " \033[41m  "$w$c" Toolname: "$p"adit framework   \033[44m  "$w$c" Author: "$p"iqbalmh18"
